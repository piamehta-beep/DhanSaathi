...
export function BarRow({ feature, shap, direction, max }: {
  feature: string; shap: number; direction: string; max: number;
}) {
  ...
  return (
    <li className="flex flex-col gap-1">
      <div className="flex items-baseline justify-between gap-3">
        <span>{label}</span>
        <span className={"shrink-0 text-sm font-semibold " + (raises ? "text-care-ink" : "text-safe-ink")}>
          {raises ? t("why.raises") : t("why.lowers")}
        </span>
      </div>
      <div className="h-3 w-full rounded-full bg-sand" aria-hidden>
        <div
          className={"h-3 rounded-full transition-[width] duration-200 ease-out " + (raises ? "bg-care" : "bg-safe")}
          style={{ width: `${width}%` }}
        />
      </div>
    </li>
