...
class SupportRequest(Base):
    ...
    linked_audit_log_id = Column(UUID(as_uuid=True), ForeignKey("audit_logs.id"), nullable=True)
    status = Column(String(15), nullable=False, default="open", server_default="open")
    created_at = Column(DateTime(timezone=True), server_default=func.now())

    __table_args__ = (
        CheckConstraint("reason IN ('low_confidence','vetoed_recommendation','general_question','dispute')", name="ck_support_reason"),
        CheckConstraint("status IN ('open','resolved')", name="ck_support_status"),
        Index("idx_support_customer_created", "customer_id", "created_at"),
    )
